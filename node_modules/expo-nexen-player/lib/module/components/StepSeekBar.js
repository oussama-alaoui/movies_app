...
import { Animated, Easing, I18nManager, PanResponder, StyleSheet, Text, View } from 'react-native';
...
const StepSeekBar = props => {
  ...
  const panResponder = React.useRef(PanResponder.create({
    ...
    onPanResponderMove: (e, gestureState) => {
      if (dx.current != gestureState.dx) {
        ...
        if (isMoving.current) {
          const position = boundPosition(locationX.current + dx.current, seekDistance.current);
  ...
  }, data.map((d, i) => /*#__PURE__*/React.createElement(View, {
    key: i,
